...
export default function RecentOrders({ orders, onViewOrder, onPrintOrder, onRefresh }: RecentOrdersProps) {
    ...
    const getTypeIcon = (type: string) => {
        switch (type) {
            case 'TAKEAWAY': return <ShoppingBag className="h-6 w-6" />
            case 'DINE_IN': return <Utensils className="h-6 w-6" />
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 mt-1">
                                        {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • {order.customerName} • {order.type}
                                    </p>
                                </div>
                            </div>
