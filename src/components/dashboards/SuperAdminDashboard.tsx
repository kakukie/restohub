...
import { BarChart3, Users, Utensils, DollarSign, LogOut, Plus, Edit, Trash2, Search, ArrowUpRight, ArrowDownRight, Shield, Save, CheckCircle, Smartphone, Megaphone, Building2, Store, TrendingUp, ShoppingBag, Zap } from 'lucide-react'
...
export default function SuperAdminDashboard() {
  ...
  const handleEditRestaurantAdmin = (restaurant: Restaurant) => {
    const adminUser = users.find(u => u.email === restaurant.adminEmail)
    if (adminUser) {
      setEditingUserData({
        id: adminUser.id,
        name: adminUser.name,
  ...
  const handleSaveRestaurant = () => {
    ...
    if (editingRestaurant) {
      updateRestaurant(editingRestaurant.id, restaurantForm)
      ...
      const newResto: Restaurant = {
        ...
        status: 'ACTIVE',
        adminEmail: restaurantForm.adminEmail || '',
        rating: 5.0,
        slug: (restaurantForm.name || '').toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, ''),
