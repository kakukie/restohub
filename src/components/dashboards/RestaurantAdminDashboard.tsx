...
export default function RestaurantAdminDashboard() {
  ...
  // Simulate Order Notifications - Robust Version
  useEffect(() => {
    ...
    if (prevPendingCount === 0 && currentPendingOrders.length > 0) {
      setPrevPendingCount(currentPendingOrders.length)
      return
    }

    // New order detected
    if (currentPendingOrders.length > prevPendingCount) {
      const newCount = currentPendingOrders.length - prevPendingCount
      toast({
      ...
      // Play a notification sound if possible (user interaction required usually, but we can try)
      // const audio = new Audio('/notification.mp3'); audio.play().catch(e => console.log('Audio play failed', e));
    }
  ...
  useEffect(() => {
    ...
    // Assuming PENDING is potential revenue, strictly speaking usually only CONFIRMED/COMPLETED counts,
    // but for simple dashboard we often sum non-cancelled.
    // User asked: "order yang dicancel masukan dalam analitik order cancel serta total reveneu cancel"
    ...
    setStats({
      ...
      totalOrders: orders.length, // Total volume including cancelled
      totalRevenue: validOrders.reduce((acc, curr) => acc + curr.totalAmount, 0),
      cancelledOrders: cancelledOrdersList.length,
      cancelledRevenue: cancelledOrdersList.reduce((acc, curr) => acc + curr.totalAmount, 0)
    })
  }, [menuItems.length, categories.length, orders.length])

  // Sub-component for Settings
  function RestaurantSettingsForm({ restaurantId }: { restaurantId: string }) {
